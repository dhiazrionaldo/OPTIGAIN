"""
OptiGain AI Product Mix Simulator — v8.4.1
Revamp dari v8.4.0

Perubahan utama:
  - FIX: build_ledger_from_snapshot() — fungsi baru sebagai pengganti
         build_product_ledger() berbasis historical average. Baseline ledger
         kini menggunakan last month QTY × unit_price/cogs dari product_master
         (via global product index / gp), sehingga retained products akan
         memiliki proj_* yang identik dengan last month actuals.
  - FIX: simulate_product_mix() — gunakan build_ledger_from_snapshot() jika
         data last month tersedia, fallback ke build_product_ledger() untuk
         customer tanpa data last month (misal customer baru).
  - GUARANTEED: Semua fitur & output field names v8.4.0 tidak berubah sama sekali.

Kenapa pakai product_master sebagai sumber harga (bukan last month actuals):
  - product_master.average_selling_price & cogs_unit adalah harga standar
    yang sudah disepakati, bebas dari anomali diskon/batch tertentu bulan lalu.
  - gp sudah memprioritaskan catalog (product_master) via load_product_catalog(),
    sehingga tidak perlu akses DB tambahan.
  - Fallback ke computed dari last month jika produk belum ada di product_master.
"""
...
app = FastAPI(title="OptiGain AI Product Mix Simulator", version="8.4.1")
...
def build_global_product_index(records: List[dict], catalog: Dict[str, Dict] = None) -> dict:
    ...
    for spec, d in raw.items():
        ...
        # product_master sebagai sumber utama; fallback ke computed dari transaksi
...
def build_product_ledger(raw_products, c_months):
    """
    Fallback: bangun ledger dari historical average (dipakai jika tidak ada
    data last month untuk customer tersebut, misal customer baru).
    """
    ledger = {}
...
def build_ledger_from_snapshot(lm_products: dict, gp: dict) -> dict:
    """
    NEW v8.4.1: Bangun ledger dari last month QTY × harga standar product_master (via gp).

    Kenapa pakai gp (product_master) sebagai sumber harga, bukan last month actuals:
      - product_master.average_selling_price & cogs_unit = harga standar yang bebas
        dari anomali diskon/batch tertentu bulan lalu.
      - gp sudah memprioritaskan catalog via load_product_catalog(), tidak perlu
        akses DB tambahan.
      - Fallback ke computed dari last month jika produk belum ada di product_master.

    Efek pada retained products:
      - proj_qty  == curr_qty  == last month qty   → qtyChange == 0 ✓
      - proj_sales == curr_sales (qty × std price)  → tidak ada perubahan   ✓
      - proj_cogs  == curr_cogs  (qty × std cogs)   → tidak ada perubahan   ✓
    """
    ledger = {}
    for spec, d in lm_products.items():
        ...
        if gp_info is not None:
            # Utama: harga standar dari product_master (sudah ada di gp)
            unit_price = gp_info["unit_price"]
            unit_cogs  = gp_info["unit_cogs"]
        else:
            # Fallback: computed dari last month actuals (produk belum di product_master)
...
async def simulate_product_mix(payload: SimulateRequest):
    ...
    for c_id, raw in customer_raw.items():
        ...
        # ── NEW v8.4.1: ambil last month snapshot untuk customer ini ──────────
        c_lm          = last_month_all.get(c_id, {})
        c_lm_products = c_lm.get("products", {})
        c_lm_key      = c_lm.get("month_key")

        # Gunakan last month sebagai baseline ledger jika tersedia.
        # Harga satuan (unit_price & unit_cogs) selalu dari product_master via gp.
        # Fallback ke historical average jika customer tidak punya data last month.
        if c_lm_products:
