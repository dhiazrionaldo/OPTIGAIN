"""
OptiGain AI Product Mix Simulator — v8.4.0
Revamp dari v8.3.0

Perubahan utama:
  - FIX: visited_pairs di run_optimization_engine() — cegah circular swap
         (misal DG 5mm → DG 4mm → DG 5mm → ...) yang menyebabkan shiftedQty
         membengkak tidak wajar (contoh: 17909.84 seharusnya 73.74).
  - FIX: apply_full_swap() & apply_pareto_partial_swap() — simpan src_unit_price
         di move dict agar format_shift_cards() bisa kalkulasi delta.asp dengan benar.
  - FIX: format_shift_cards() — pakai mv["shifted_qty"] langsung dari move
         (bukan re-snapshot ledger), sehingga shiftedQty == toProduct.projected.qty
         untuk full swap.
  - FIX: generate_ai_reasoning_batch() — arsitektur submit-all-then-poll-parallel:
         submit semua chunks dulu lalu poll paralel pakai asyncio.gather, eliminasi
         idle delay 60s antar chunk yang jadi penyebab utama proses 10+ menit.
  - FIX: AI_BATCH_CHUNK_DELAY_SEC dihapus, diganti AI_BATCH_SUBMIT_DELAY_SEC = 0
         (karena semua sudah masuk 1 batch jika ≤50 customers butuh AI).
  - RETAINED: Semua fitur v8.3.0 (product catalog, pareto, last month snapshot,
              prompt caching, exponential backoff, sliding window rate limit).
...
app = FastAPI(title="OptiGain AI Product Mix Simulator", version="8.4.0")

TARGET_GM_PCT        = 9.0
SUPABASE_CHUNK       = 50
PARETO_MIN_QTY_RATIO = 0.5   # pareto products: minimal 50% qty harus dipertahankan

# ── AI Config ─────────────────────────────────────────────────────────────────
AI_MODEL                  = "claude-sonnet-4-6"
AI_MAX_TOKENS             = 300
AI_TEMPERATURE            = 0.3
AI_BATCH_CHUNK_SIZE       = 50     # semua masuk 1 batch jika ≤50 customers butuh AI
AI_BATCH_SUBMIT_DELAY_SEC = 0      # tidak perlu delay antar submit jika hanya 1 chunk
AI_BATCH_POLL_SEC         = 5      # interval polling (detik)
AI_BATCH_POLL_JITTER_SEC  = 2      # jitter antar poller paralel
AI_BATCH_MAX_WAIT         = 600    # max tunggu per batch = 10 menit
AI_BACKOFF_BASE_SEC       = 5.0    # base exponential backoff
AI_BACKOFF_MAX_SEC        = 60.0   # max backoff cap
AI_MAX_RETRIES            = 5      # max retry create batch per chunk

# ── System Prompt (di-cache agar tidak dihitung ke ITPM) ─────────────────────
AI_SYSTEM_CONTEXT = (
...
def find_family_substitution(source_spec, global_products):
...
def apply_full_swap(ledger, src_spec, tgt_spec, gp):
    """
    Pindahkan seluruh proj_qty src ke tgt.
    FIX v8.4.0: simpan src_unit_price di move dict (harga satuan SEBELUM swap)
    agar format_shift_cards bisa hitung delta.asp dengan benar.
    """
    src = ledger.get(src_spec)
    if src is None or src["proj_qty"] < 1e-6: return None

    qty       = src["proj_qty"]                          # qty yang benar-benar dipindah
    ...
    move = {
        ...
        "to":               tgt_spec,
        "shifted_qty":      fmt(qty),
        "src_unit_price":   fmt(src_price),              # ← BARU: harga satuan sumber
...
def apply_pareto_partial_swap(ledger, src_spec, tgt_spec, gp):
    """
    Pindahkan 50% proj_qty src ke tgt, sisakan 50% (PARETO_MIN_QTY_RATIO).
    FIX v8.4.0: simpan src_unit_price di move dict.
    """
    src = ledger.get(src_spec)
    if src is None or src["proj_qty"] < 1e-6: return None
    ...
    shift_qty  = total_qty - keep_qty                    # qty yang benar-benar dipindah
...
def run_optimization_engine(ledger, mapping_lookup, gp, raw_products, c_months, pareto_specs):
    """
    FIX v8.4.0: tambah visited_pairs untuk mencegah circular swap
    (misal A→B→A→B→...) yang menyebabkan shiftedQty membengkak tidak wajar.
    Setiap pasangan (src, tgt) hanya boleh terjadi SATU KALI per customer per run.
    """
    all_moves, exhausted = [], set()
    visited_pairs: Set[tuple] = set()              # ← BARU: track swap yang sudah terjadi
    ...
    for _ in range(len(ledger) * 20):
        ...
        # Filter kandidat: hapus yang sudah pernah menjadi sumber swap ke worst_spec
        # (cegah circular: kalau A sudah swap ke B, B tidak boleh swap balik ke A)
        candidates = [
        ...
        if chosen is None:
            fb = find_family_substitution(worst_spec, gp)
        ...
        # Cegah pasangan yang sama dieksekusi dua kali
...
def format_shift_cards(agg_moves, ledger, gp):
    """
    FIX v8.4.0:
    - Pakai mv["shifted_qty"] langsung dari move (bukan re-kalkulasi dari ledger snapshot)
      sehingga shiftedQty == toProduct.projected.qty untuk full swap.
    - Pakai mv["src_unit_price"] untuk kalkulasi delta.asp yang akurat.
    - Pakai gp unit_price/unit_cogs untuk delta (bukan dari ledger projected yang bisa
      ter-aggregate dari banyak swap).
    """
    cards = []
    for mv in agg_moves:
        fs, ts = mv["from"], mv["to"]
        ...
        # Harga satuan sumber sebelum swap (dari move, bukan dari ledger saat ini)
        src_asp = mv.get("src_unit_price") or (
            fc["asp"] if fc["qty"] > 1e-6 else gp.get(fs, {}).get("unit_price", 0)
        )
        # Harga satuan target dari global product index (lebih stabil)
        ...
        cards.append({
            ...
            "shift": {
                "shiftedQty":     mv["shifted_qty"],        # ← langsung dari move
...
def build_next_month_plan(ledger, gp, pareto_specs, last_mo_prods, shift_cards):
    ...
    for spec, p in ledger.items():
        lm  = last_mo_prods.get(spec, {})
        ent = {
            "spec":           spec,
            "family":         p.get("family", extract_family(spec)),
            "isPareto":       spec in pareto_specs,
            "lastMonthQty":   fmt(lm.get("qty",   0.0)),
            "lastMonthSales": fmt(lm.get("sales", 0.0)),
            "lastMonthGmPct": fmt(lm.get("gmPct", 0.0)),
            "lastMonthAsp":   fmt(lm.get("asp",   0.0)),
...
# Submit-all-then-poll-parallel (FIX v8.4.0: eliminasi idle delay antar chunk)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def generate_ai_reasoning_batch(customer_results: list) -> List[str]:
    """
    FIX v8.4.0: Submit-all-then-poll-parallel.
    1. Submit semua chunks (dengan delay antar submit jika > 1 chunk).
    2. Poll semua batch secara paralel pakai asyncio.gather.
    Eliminasi idle delay 60s antar chunk yang jadi bottleneck utama.
    """
    ...
    # Kumpulkan semua request yang butuh AI
    ...
    for chunk_idx, chunk in enumerate(chunks):
        ...
        # Rate-limit delay hanya antar submit (bukan setelah polling)
    ...
    async def poll_and_retrieve(batch_obj, chunk_idx: int) -> Dict[str, str]:
        ...
        chunk_num = chunk_idx + 1
        # Jitter kecil agar tidak semua poller burst ke API di detik yang sama
...
async def simulate_product_mix(payload: SimulateRequest):
    ...
    for c_id, raw in customer_raw.items():
        ...
        c_lm     = last_month_all.get(c_id, {}).get("products", {})
        c_lm_key = last_month_all.get(c_id, {}).get("month_key")
        nmp      = build_next_month_plan(ledger, gp, pareto_specs, c_lm, shift_cards)
...
# AI_BATCH_POLL_SEC         = 5      # interval polling (detik)
# AI_BATCH_POLL_JITTER_SEC  = 2      # jitter antar poller paralel
# AI_BATCH_MAX_WAIT         = 300    # max tunggu per batch = 5 menit
# AI_BACKOFF_BASE_SEC       = 5.0    # base exponential backoff
# AI_BACKOFF_MAX_SEC        = 60.0   # max backoff cap
# AI_MAX_RETRIES            = 5      # max retry create batch per chunk
