...
export async function triggerN8NWebhookPeriod(payload: WebhookPayload): Promise<void> {
  const webhookUrl = process.env.N8N_WEBHOOK_URL_PREDICTION_PERIOD

  if (!webhookUrl) {
    const error = "N8N_WEBHOOK_URL_PREDICTION_PERIOD is not configured"
    console.error(error)
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Response(JSON.stringify({ success: false, error: userMessage }), { status: response.status })
    }
